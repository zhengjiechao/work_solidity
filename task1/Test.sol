...
contract Test{
    ...
    function intToRoman(uint number)public pure returns(string memory){
        ...
        result = string.concat(result,intToRomanItem(divideSecond,"C","D","M"));

        uint divideThird = (number - divideFirst*1000 - divideSecond*100)/10;
        result = string.concat(result,intToRomanItem(divideThird,"X","L","C"));

        uint divideFour = number - divideFirst*1000 - divideSecond*100 - divideThird*10;
        result = string.concat(result,intToRomanItem(divideFour,"I","V","X"));

        return result;
    }

    function intToRomanItem(uint divide,string memory str1,string memory str2,string memory str3)private pure returns(string memory){
        string memory str;
    ...
    function romanToInt(string memory s) public view returns(uint){
        ...
        for(uint i = 0;i < strBytes.length; i++){
            ...
            if (i < strBytes.length -1){
                if(item == 0x49 && (strBytes[i+1] == 0x56 || strBytes[i+1] == 0x58)){
                    result -= romanToIntMap[item] * 2;
                }else if(item == 0x58 && (strBytes[i+1] == 0x4c || strBytes[i+1] == 0x43)){
                    result -= romanToIntMap[item] * 2;
                }else if(item == 0x43 && (strBytes[i+1] == 0x44 || strBytes[i+1] == 0x4d)){
                    result -= romanToIntMap[item] * 2;
                }
            }
        }
